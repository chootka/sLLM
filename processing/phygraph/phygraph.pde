...
void setup() {
  size(400, 250);
  // Position window at a reasonable location (was 2200,200 which might be off-screen)
...
void draw() {
  ...
  if(begin==false)
  {
    initialPresentation(flagFirstOrder, flagSecondOrder, flagRemoveSave, flagRecord, flagSaveVolts, flagThirdOrder);
    startTime=millis();
  ...
  if(begin==true)
  {
   ...
   //println(counterSamples);
    ...
    if(frameCount %100==0)
    {
      CurveA.sendHertz(hertz, hertzArray, viewHertzSize);
    }
    }
    ...
    drawPresentation(CurveA.XR,CurveA.XOut,twoSampleView,elapsedTime,samples,durationVal,samplesPerSecond,sliderValue,counterSamples,hertz,N,folderName,captureImgName,orderOfMagnitude,viewHertzSize,hertzArray);

    //Send FFT data to database
   iteration = createDatabase(CurveA.XOut,twoSample,N,toggleValue,outputFile,outputFileData,iteration);
    secEnd=second();
    minEnd=minute();
 ...
 //  sliderValue=cp5.get(Slider.class,"Time of experiment").getValue();
  ...
  // Use File.separator for cross-platform compatibility, or "/" which works on all platforms in Processing
