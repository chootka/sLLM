...
| `--amp` | `1.0` | Sends the organism's rise and fall to **level** as well as pitch, per channel. Whichever electrode is being pushed hardest is the voice that comes up, so a contraction is a swell and not only a bend. At 1.0 the piece breathes on the organism's own cycle -- about 8 s at speed 12 -- over 4 to 8 dB depending on how active the passage is. Each electrode is measured against its own recent range rather than a fixed scale, so quiet stretches still breathe and busy ones are not flattened; nothing is invented, only the scale changes. `AMP_FLOOR`, `AMP_OPEN`, `AMP_CLOSE` and `AMP_MINSPAN` in the worklet tune that window. |
| `--ring` | `0.20` | The real sum and difference tones, by multiplying the oscillators against each other. Where `--ghost` reconstructs one artifact from measured frequencies, this **is** the interaction, taken off the signals -- so all the swooping comes free. Present in the bed as well as at contact: a bed of one low tone with nothing moving above it reads as a broken speaker rather than a choice, and these wander constantly. At 0.20 the 210 Hz product sits at 0.27 of the fundamental -- clearly audible, under the bed rather than on top of it. |
